...
from sprites import *
from settings import IMG_DIR, AUDIO_DIR, DATA_DIR
...
class Game:
    def __init__(self):
        ...
        # --- icon ---
        icon = pygame.image.load(os.path.join(IMG_DIR, 'icon.png')).convert_alpha()
        ...
        # --- sounds ---
        pygame.mixer.music.load(os.path.join(AUDIO_DIR, '8-Bit-Indigestion.ogg'))
        pygame.mixer.music.set_volume(0.5)
        pygame.mixer.music.play(-1)

        self.damage_sound = pygame.mixer.Sound(os.path.join(AUDIO_DIR, 'damage.ogg'))
        self.death_sound = pygame.mixer.Sound(os.path.join(AUDIO_DIR, 'death-sound.ogg'))
        self.record_sound = pygame.mixer.Sound(os.path.join(AUDIO_DIR, 'new_record_sound.ogg'))
        self.ability_sound = pygame.mixer.Sound(os.path.join(AUDIO_DIR, 'ability.ogg'))
    ...
    def run(self):
        while self.running:
            ...
            stats_text = self.font1.render(
                f"Lives: {STATS['health']}  Score: {STATS['score']}  Record: {STATS['record']}",
                True, "black"
            )
        ...
        game_over_message = self.font2.render("Game Over!", True, "red")
